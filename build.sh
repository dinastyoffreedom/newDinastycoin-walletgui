...
DINASTYCOIN_DIR=dinastycoin
DINASTYCOIND_EXEC=dinastycoind
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/dinastycoin-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    DINASTYCOIND_EXEC=dinastycoind.exe
fi

# force version update
get_tag
# echo "var GUI_VERSION = \"$TAGNAME\"" > version.js
echo "var GUI_VERSION = \"v3.21.0.0-46-g5bc99ef\"" > version.js
pushd "$DINASTYCOIN_DIR"
get_tag
popd
echo "var GUI_DINASTYCOIN_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../dinastycoin-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy dinastycoind to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$DINASTYCOIN_DIR/bin/$DINASTYCOIND_EXEC $BIN_PATH
fi
# make deploy
popd
