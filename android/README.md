...
# Base environnement
        ...
        cd dinastycoin/utils/build_scripts
        docker build -f android32.Dockerfile -t dinastycoin-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t dinastycoin-gui-android .
        docker create -it --name dinastycoin-gui-android dinastycoin-gui-android bash

# Get the apk

        docker cp dinastycoin-gui-android:/opt/android/dinastycoin-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/dinastycoin-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
